...
const setLocalStorageContext = (AccountContext: Account) => {
  localStorage.setItem('ACCOUNT_CONTEXT', JSON.stringify(AccountContext))
}
export interface UserContextProps {
  setSimpleAccountApi: React.Dispatch<
    React.SetStateAction<SimpleAccountAPI | null>
  >
...
export function UserContextProvider({
  ...
  const [simpleAccountApi, setSimpleAccountApi] =
    useState<SimpleAccountAPI | null>(null)
  const setUserContext = (accountContext: Account) => {
    try {
  ...
  useEffect(() => {
    if (localStorage.getItem('ACCOUNT_CONTEXT')) {
      setUserContext(
        JSON.parse(localStorage.getItem('ACCOUNT_CONTEXT')!) as Account
      )
    }
  }, [])
  useEffect(() => {
