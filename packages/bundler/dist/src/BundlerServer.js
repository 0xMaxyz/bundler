...
class BundlerServer {
    ...
    async _preflightCheck() {
        ...
        try {
            await utils_2.IEntryPoint__factory.connect(this.config.entryPoint, this.provider).callStatic.getUserOpHash((0, utils_2.packUserOp)(emptyUserOp));
        }
        catch (e) {
            this.fatal(`Invalid entryPoint contract at ${this.config.entryPoint}. wrong version? ${(0, utils_2.decodeRevertReason)(e, false)}`);
        }
        const signerAddress = await this.wallet.getAddress();
        const bal = await this.provider.getBalance(signerAddress);
        this.log('signer', signerAddress, 'balance', ethers_1.utils.formatEther(bal));
        if (bal.eq(0)) {
